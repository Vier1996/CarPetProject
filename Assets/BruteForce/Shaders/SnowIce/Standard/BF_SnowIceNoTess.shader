...
Shader "BruteForce/Standard/SnowIceNoTessellation" {
	...
	Properties{
		...
		[MainColor]_Color("Snow Tint", Color) = (0.77,0.86,0.91,1)
...
#include "UnityCG.cginc"
				...
				ENDCG
					...
					float _SpecForce, _LittleSpecForce, _UpVector, _NormalVector, _IceScale, _SnowScale;
					...
					InterpolatorsVertex vert(VertexData v) {
						...
						snowHeightNew = lerp(snowHeightNew, tex2Dlod(_Mask1, float4(originalPos.xz, 0, 0) * _HeightScale * 0.1).r, splat_control.g * (1 - _Metallic1));
						snowHeightNew = lerp(snowHeightNew, tex2Dlod(_Mask2, float4(originalPos.xz, 0, 0) * _HeightScale * 0.1).r, splat_control.b * (1 - _Metallic2));
						snowHeightNew = lerp(snowHeightNew, tex2Dlod(_Mask3, float4(originalPos.xz, 0, 0) * _HeightScale * 0.1).r, splat_control.a * (1 - _Metallic3));
...
_LittleSpecForce = pow(_LittleSpecForce, gamma) * 1.5;
					...
					_Specular3 = pow(_Specular3,gamma);
								...
								float3 rippleMain = tex2D(_GlobalEffectRT, uv);
								float3 rippleMainAdditional = tex2D(_GlobalEffectRTAdditional, uvAdd);
								...
								SnowNormal0 = lerp(SnowNormal0,  SnowNormal0 * _NormalMultiplier * 2 * iceValue - i.normal, 1 - _Metallic0);
								float3 SnowNormal1 = _Normal1.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat1_ST.xy));
								SnowNormal1 = lerp(SnowNormal1,  SnowNormal1 * _NormalMultiplier * 2 * iceValue - i.normal, 1 - _Metallic1);
								float3 SnowNormal2 = _Normal2.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat2_ST.xy));
								SnowNormal2 = lerp(SnowNormal2,  SnowNormal2 * _NormalMultiplier * 2 * iceValue - i.normal, 1 - _Metallic2);
								float3 SnowNormal3 = _Normal3.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat3_ST.xy));
								SnowNormal3 = lerp(SnowNormal3,  SnowNormal3 * _NormalMultiplier * 2 * iceValue - i.normal, 1 - _Metallic3);

								// SNOW NORMAL //
								float3 normal = lerp(0, SnowNormal0, splat_control.r * (1 - _Metallic0));
								normal = lerp(normal, SnowNormal1, splat_control.g * (1 - _Metallic1));
								normal = lerp(normal, SnowNormal2, splat_control.b * (1 - _Metallic2));
								normal = lerp(normal, SnowNormal3, splat_control.a * (1 - _Metallic3));
								...
								snowColor = lerp(snowColor, SnowSplat1, splat_control.g * (1 - _Metallic1));
								snowColor = lerp(snowColor, SnowSplat2, splat_control.b * (1 - _Metallic2));
								snowColor = lerp(snowColor, SnowSplat3, splat_control.a * (1 - _Metallic3));
					...
					#endif
								IceTex.rgb = IceTex.rgb * _LightColor0.rgb;
								...
								if (parallaxDepth < 0)
								{
									albedo.rgb = lerp(albedo.rgb, _ParalaxColor.rgb, saturate(abs(parallaxDepth)) * saturate(1 - pow(iceValue, 5)));
								}
								else
								{
...
#ifdef USE_AL
								half3 ambientColor = ShadeSH9(half4(lerp(normalDirection, normalDirection + normalEffect * 2.5, saturate(ripples + ripples3)), 1));
#if !UNITY_COLORSPACE_GAMMA
