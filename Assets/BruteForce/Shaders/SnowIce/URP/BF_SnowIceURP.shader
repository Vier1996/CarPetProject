...
half4 lightColor = half4(mainLight.color.rgb, (mainLight.color.r + mainLight.color.g + mainLight.color.b)/3);
					...
					_Specular3 = pow(_Specular3, gamma);
								...
								//float3 normal = SampleNormal(i.uv * _SnowNormalScale * _SnowScale, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap)).rgb * iceValue - i.normal;
								...
								//SampleNormal(i.uv* _IceScale, TEXTURE2D_ARGS(_NormalTex, sampler_BumpMap));
								half3 normalSample = UnpackNormalScale(_BumpMap.Sample(sampler_BumpMap, i.uv * _SnowScale), _NormalMultiplier * 3).rgb;
								
								//worldNormal.x = dot(normalDirection.x, lerp(normalTex, SampleNormal(i.uv * _SnowScale, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap)) * 3 * _NormalMultiplier, iceValue));
								//worldNormal.y = dot(normalDirection.y, lerp(normalTex, SampleNormal(i.uv * _SnowScale, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap)) * 3 * _NormalMultiplier, iceValue));
								//worldNormal.z = dot(normalDirection.z, lerp(normalTex, SampleNormal(i.uv * _SnowScale, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap)) * 3 * _NormalMultiplier, iceValue));
